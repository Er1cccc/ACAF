...
public class CoreMethodAdapter<T> extends MethodVisitor {
    private HashMap<MethodReference.Handle,Set<Integer>> PASSTHROUGH_DATAFLOW=new HashMap<>();
    ...
    private void initPassthrough() {
        Object[][] passthrough= new Object[][] {
                ...
                { "java/net/HttpURLConnection", "getInputStream", "()Ljava/io/InputStream;", 0 },


                {"org/apache/http/client/methods/HttpGet","<init>","(Ljava/lang/String;)V",1}
        };
        for (Object[] pt : passthrough) {
            MethodReference.Handle handle = new MethodReference.Handle(new ClassReference.Handle((String) pt[0]), (String) pt[1],(String) pt[2]);
            Set<Integer> argInd=new HashSet<>();
            for(int i=3;i<pt.length;i++){
                argInd.add((Integer) pt[i]);
            }
            PASSTHROUGH_DATAFLOW.put(handle,argInd);
        }
    }
    ...
    public void visitMethodInsn(int opcode, String owner, String name, String desc, boolean itf) {
        ...
        switch (opcode) {
            ...
            case Opcodes.INVOKEINTERFACE:
                ...
                Set<Integer> passthrough = PASSTHROUGH_DATAFLOW.get(methodHandle);
                if(passthrough!=null){
                    for (int i:passthrough) {
                        resultTaint.addAll(argTaint.get(i));
                    }
                }
